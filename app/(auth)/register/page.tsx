  ...
  useEffect(() => {
    ...
    if (errorParam) {
      const errorMessages: Record<string, string> = {
        'OAuthSignin': 'Failed to sign up. Please try again.',
        ...
        'EmailSignInError': 'Unable to sign up.',
        'CredentialsSignin': 'Invalid credentials.',
        'AccessDenied': 'Access denied.',
        'Verification': 'Verification failed.',
      };
      const message = errorMessages[errorParam] || 'Registration failed. Please try again.';
      setError(message);
      toast.error(message);
    }
  }, [searchParams]);
    ...
    >
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-12 relative overflow-hidden">
        ...
        <div className="space-y-4 relative">
          <h2 className="text-3xl font-black text-white leading-tight" style={{ fontFamily: 'Outfit, sans-serif' }}>
            Everything you need to land your{' '}
            <span className="text-indigo-400">dream job</span>
          </h2>
          {[
            { icon: '🤖', text: 'AI rewrites your bullet points for impact' },
            { icon: '🎯', text: 'ATS score checker included free' },
            { icon: '📄', text: '30+ professional templates ready to use' },
            { icon: '⚡', text: 'PDF export in under 3 seconds' },
          ].map((item, i) => (
      ...
      <div className="flex flex-col justify-center w-full lg:w-1/2 px-6 py-12">
        <div className="w-full max-w-md mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-white/50 text-sm">Start building resumes that get you hired</p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 mb-6">
            <button
              ...
              disabled={isLoading}
              className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-medium text-white/80 border border-white/10 bg-white/3 hover:bg-white/6 hover:border-white/15 transition-all duration-200 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
